...
//测量距离
Sandcastle.addDefaultToolbarButton("测量距离", function () {
    measureLineSpace(viewer);
})
//测量面积
Sandcastle.addDefaultToolbarButton("测量面积", function () {
    measureAreaSpace(viewer);
})
//测量面积
Sandcastle.addDefaultToolbarButton("清除", function () {
...
function addSources() {
    ...
    viewer.dataSources.add(
        Cesium.CzmlDataSource.load("../cesium/simple.json")
    );
    //定位
    viewer.camera.flyTo({
        destination: Cesium.Cartesian3.fromDegrees(115.7, 39.4, 30000000),//经度、纬度、高度
        orientation: {
